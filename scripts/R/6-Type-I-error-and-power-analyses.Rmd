...
Please be aware that the code executes a good number of sampling-based simulations, and these simulations take some time to complete. The parameter that determines how many samples are used in each of the sampling simulations is `n.samples`. You can change the value of `n.samples` to a small number (e.g., `10`) to speed up compilation.

# Sampling-based Type I error and power analyses 
In the previous [lecture on sampling-based approaches](https://github.com/tfjaeger/BCS206-207-R-lectures/blob/2024-PhD-skillz-workshop/5-Sampling-based-approaches-lecture.Rmd), we have introduced sampling-based approaches as an alternative to analytical approaches. We have learned how they can be used to understand statistical concepts (like the standard error of the mean) r to calculate, e.g., confidence intervals while avoiding parametric assumptions.

Another powerful application of sampling-based approaches is to obtain estimates of the **Type I error** rate (how often the analysis returns significance when in reality there is no effect in the underlying population) or **Type II error** rate of an analysis approach (how often we fail to find an effect that is actually present in the underlying population). These error rates depend on both:

 1. the *data*, including not just the amount and grouping structure of the data, but also assumptions made about the distributions in the data (this point is often overlooked)
 2. the *analysis approach*, including assumptions it makes about the distribution of the outcome (e.g., Normally vs. Bernoulli distributed), the relevant predictors, and the relation between predictors and the outcome (e.g,. linearity). 
 
That is, while one analysis approach might yield the targeted Type I error rate (e.g., a Type I error rate of .05 for a significance criterion of $\alpha = .05$), another approach might fail to achieve this error rate on the same data. Conversely, approach A might work better for data set 1, whereas approach B works better for data set 2 (where "better" means achieving an error rate closer to the targeted error rate). 

For Type I errors, we want to avoid in particular *anti-conservativity*---error rates above .05---but also *conservativity*---error rates lower than .05. For **power** (which is simply 1 - Type II error), we want to have as much statistical power as possible. Whereas conventions for targeted Type I error rates tend to be pretty clear, conventions for what constitutes sufficient power tend to be less clear. 

In this lecture, we go through how one can use the sampling-based approach to assess power and Type I error rates. These analyses are based conducted before collecting data (e.g., so that one can still change how much data to collect), but can also be conducted after data collection prior to the analysis (e.g., to guide which analysis approach one should choose), or even after analyses are already conducted (e.g., to assess how much we can trust our results).
...
For **step (ii)**, we write a function that can generate (i.e., sample) RT and accuracy data from these parameters. For the RTs we assume that they are drawn from a normal distribution with the mean and SD observed in our experiment for Subject ```r which.subj```. For accuracy, we assume that it is drawn from a Bernoulli distribution with the mean observed in our experiment for Subject ```r which.subj```. Since we are conducting a Type I error simulation, we are *not* adding a difference to RTs and accuracies of the two conditions (or, equivalently, we are adding 0 as a difference).
...
make_single_subject_data = function(n.trial, d.pars) {
  ...
  crossing(
    ...
    # from.
    mutate(
      correct = rbinom(n = nrow(.), size = 1, prob = d.pars$correct_mean[1]),
      RT = rnorm(n = nrow(.), mean = d.pars$RT_mean[1], sd = d.pars$RT_sd[1])
    ) %>%
    # Do some formatting and sort the data by subject, trial, and 
    # condition. These lines of code are not strictly necessary.
    as_tibble() %>%
    mutate_at(c("condition", "subject"), factor) %>%
    arrange(subject, trial, condition)
}
...
Here's what happens if we chain steps (ii) and (iii) together to draw single new instance of the experiment and apply the $t$-tests:
...
For **step (iv)**, we repeat the step (ii) and (iii) 1000 times, giving us the following (showing only the first 20 rows). Note that there are some `NA`s in the output. That's the case because the t-test sometimes doesn't converge:
...
# ning each of the .n repetitions of the r code. rdply() is this similar 
# to the function replicate() but it conveniently also binds the output 
...
Finally, for **step (v)**, we calculate the proportion of the 1000 samples for which the effects of conditions on RTs or accuracy were significant, which corresponds to the estimated Type I error rate of the $t$-tests (since we simulated a ground truth that did *not* add an effect):
...
These Type I error rates look OKish for the RT analysis, but clearly conservative for the analysis of accuracy. Conservative Type I error rates are not necessarily bad---in science, it's typically better to be conservative than anti-conservative. However, conservative Type I error rates can also indicate that one is (also) wasting power, and that can be problematic (in particular, for replication studies that find replication failures)!

As mentioned in the footnote above, any Type I error calculation is only as good as its assumptions. In this particular case, we've made some problematic assumptions. For example, we assumed that RTs are drawn from a normal distribution, but that would predict that there can be negative RTs (the tails of a normal distribution are infinitely long). Such problematic assumptions do not *necessarily* lead to wrong estimates of the Type I error rate, but they *can* and that should worry us (see Burchill & Jaeger, 2023 for large-scale Type I error and power simulations for reading time data). 
...
These results suggest that we might not want to use a $t$-test for the analysis of our accuracy results. Indeed, this problem of $t$-tests and ANOVA for the analysis of categorical outcomes is well-known (see Dixon, 2008; Jaeger, 2008, both in a special issue on statistical methods in the *Journal of Memory & Language*; for sociolinguists, Johnson, 2009 in *Language Variation & Change* provides a great introduction with intuitive examples and excellent illustrations). But even if we didn't know anything about it, the sampling-based approach has provided us with a tool to notice that something might be off.

## What if we don't know the ground truth? Bootstrap!
In the above example, we knew the ground truth and this let us assess whether the assumptions we made for our Type I error analysis were problematic. But that is rarely the case. How then, can we avoid problematic assumptions? While there is no magic bullet---nothing that will always work---the non-parametric bootstrap we've used above to obtain confidence intervals also allows us to get Type I error estimates, without assuming, e.g., normality. Specifically, we can bootstrap from the data we have. Specifically, we can substitute steps (i) and (ii) by sampling without replacement data points from subject `r which.subj`. 
...
Applying the same steps (iii) - (v) as above, we find that the Type I error rate of both analyses seems similar to what we obtained above by generating data under the (in this case) known ground truth:
...
This demonstrates how the bootstrap approach offers a powerful approach to Type I error (and power) analyses when the ground truth is *not* known (i.e., most of the time) and there is reason to expect that the parametric assumptions made by our analyses are problematic (which is the case for many of the outcome measures collected in the cognitive, neuro, and language sciences). See Burchill & Jaeger (2023) for an application of bootstrap resampling for Type I and II error analyses of reading time data that demonstrates when standard analysis approaches to reading times are problematic.


## Power
To calculate power, we proceed in parallel to the approach described above for Type I error rates. For power, too, we can use parametric or non-parametric approaches. The following example shows the parametric approach to simulated the data from Subject ```r which.subj```. We can modify the function used above, so that it can accommodate both Type I error analyses (by setting the argument `effect = 0`) and power analyses (by setting `effect` to some value other than 0). For the sake of simplicity, we focus on only RTs.
...
With this simple change in the data generation---i.e., step (i)---we can calculate the power for, e.g., a combination of different amounts of data (12, 24, 48, or 96 trials) and different effect sizes (25, 50, and 100 ms).^[Notice that this output immediately shows why the assumption of normality can be problematic for RTs. RTs are always positive (aside of measurement errors) but the normal distribution has non-zero probability mass even arbitrarily far from its mean. There is thus a non-zero chance that negative RTs are generated when sampling from a normal distribution even if the mean of that distribution is much larger than zero.]
